...
missing_value <- lapply(data,function(x) map_dbl(x,~sum(is.na(.))))
```
...
These transactions tracked purchases over a two year period and total over `r nrow(transactions)` specific product purchases were recorded. Total `r nrow(transactions)` specific items purchases were
recorded across `r unique_value_transactions['store','number_of_unique_values']` unique stores. A total of `r unique_value_transactions['upc','number_of_unique_values']` different products within the four commodities were recorded during this time period. 
...
Through the deeper observations, we could find that in week 12, purchases in GA dropped greatly in second year. In week 27, purchases in KY and TN increased greatly. 
...
commodity_items_plot <- dat %>% 
  ...
  theme_bw()+
  theme(axis.text.x=element_text(angle=0, hjust=1))+
  ggtitle('Total Purchases Purchases Comparision (by Commodity)')+
  theme(plot.title = element_text(hjust = 0.5),
        panel.border = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
The household penetration of each product within each commodity shows us a simple view of preferences within our grocery shopper population. There are a total `r unique_value_transactions['household','number_of_unique_values']` distinct households within this
data set, and the following table shows how many distinct households within this total
...
$ The number of repeat customers/ The number of total customers $
...
### Aunt Jemima
```{r warning=FALSE, message=FALSE, echo=FALSE,results='hide'}
aunt.association.rules <- apriori(tr, parameter = list(sup = 0.01, conf = 0.01,target="rules",minlen=1),appearance = list(default="lhs",rhs="Aunt Jemima"))
aunt.association.rules <- sort(ragu.association.rules, by='confidence', decreasing = TRUE)
aunt <- inspect(head(aunt.association.rules[1:3]))
aunt$lhs <- as.character(aunt$lhs)
 
```


For Aunt Jemima, associated brands are `r aunt[1,'lhs']` and `r aunt[2,'lhs']`.
```{r warning=FALSE, message=FALSE, echo=FALSE}
knitr::kable(head(aunt,n=5))   
```